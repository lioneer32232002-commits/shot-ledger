...
- **連莊**：一輪達到該球種門檻（罰球／上籃 8、2 分 6、3 分 5、深 3 4，每 10 球）
  就算一莊，連續達標的輪數即時顯示。挑戰確定沒過的那一節，下半場還有東西可追。
- **誠實機制**：輪次時間戳判定真實練習節奏，刷太快的成績不列入解鎖評估；
  徽章綁定出席與累計進球，不綁單節命中率。生涯數字與進球徽章都不算上籃
  （命中率天生偏高，算進去等於開後門刷數字）。
...
- **深色模式**：自動跟隨系統＋手動切換（設定頁「外觀」三段式），重整頁面記住偏好。
- **可安裝 PWA、離線可用**：加到主畫面即開即用，Service Worker network-first
  快取核心資源，離線也能記錄。
