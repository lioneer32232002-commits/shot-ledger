...
## M6.2（2026-07-14）：四分頁照片橫幅 ＋ 首頁字標放大

### 1. 為什麼是橫幅不是滿版底圖

使用者原本想要滿版底圖。這四頁都是**資料密集頁**（熱區球場圖、命中率長條、
熱力格日曆、紀錄列表、數字），照片壓在資料底下會犧牲可讀性，且淺色模式實質消失
（整站被迫變深色）。折衷：**照片放在標題區橫幅、內容區維持紙感卡片**——質感給到，
可讀性零風險。使用者同意先照此做。

### 2. 裁切（使用者的核心疑慮：手機直式 vs 桌面橫式）

**這個問題在四分頁不存在**：App 本體是 `max-width: 480px` 的置中欄，桌面版不會變寬，
所以橫幅在手機與桌面是同一個比例，一組裁切兩邊都準。只有首頁 landing 是滿版
（100dvh），那裡才有直/橫差異，而它的圖是 3:4 直式、`object-fit: cover`，主體在中段，
兩邊都吃得住。

`assets/tabbg/*.jpg`，各 1000×420 q78（共約 160KB）。裁切**不是置中，而是對準主體**
（PowerShell + System.Drawing，`cy` = 主體在來源圖高度的比例）：

| 分頁 | 來源 | cy | 內容 |
|---|---|---|---|
| train | davide-aracri-YF9YeTmhy6o | 0.47 | 球正穿過籃網（暗、戲劇性） |
| stats | jordan-tan-R0S2Kq6-_3Q | 0.59 | 木地板上的球（初版 0.78 把球切一半，已重裁） |
| history | richard-bagan-SmQ2Cku3alc | 0.52 | 靜置的 Wilson 球 |
| settings | klara-kulikova-QGinARmCGbE | 0.44 | 球館籃框與觀眾席 |

**kenjiro-koya-32eV0aDY3Ww 未採用**：4000×6000 直式，球與籃網在垂直方向拉得很開，
裁成 1000×420 必然砍掉球或砍掉網。留待日後做直式滿版底圖時使用。

### 3. 壓字層（依圖配，不統一壓黑）

基準 scrim = 左下 radial + 底部 linear。**紀錄／統計是亮圖**（拋光木地板高光，
最亮處白字對比僅 2.45／3.53），各自加強左下角的 radial 與 linear
（`.page-banner--history` / `.page-banner--stats`），只壓文字所在那一角，
右半邊照片維持亮度。

### 4. 首頁字標放大

`clamp(26px, 3.2vw, 38px)`（原 19px 固定，桌面上太小）、字重 800、
sub 改為上方一條分隔線 + `letter-spacing: .34em`（並用負 margin 補掉尾字後的多餘一格）。

### 5. SW `CACHE_NAME` v11 → **v12**（新增 js/pagebanner.js ＋ 四張 tabbg）
