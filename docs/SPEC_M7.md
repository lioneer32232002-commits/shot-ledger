...
### 分享卡 5 球種極端組合掃描（2026-07-17，HANDOFF 遺留待辦）

情境：free 場次同時有 2分/3分/深3/罰球/上籃（右欄 5 行，超出設計目標的
「最多 4 行」）＋強制 2 徽章＋照片模式（純黑底圖，前景用亮度門檻偵測墨水）。
逐 y 垂直帶掃描結果——**通過**：

- 全寬區塊間 gap：品牌→菜單 34／菜單→主數字帶 30／球種列→球場 38／
  球場→徽章 38／徽章→網址 34，全數 ≥16px。
- 右欄 5 行行間 19〜20px、左欄大字→投中行 22px（分欄掃，皆 ≥16px）。
- 全寬掃出現一處 gap 7 為方法論預告的**跨欄誤報**（右欄第 3 行底 vs 左欄
  投中行頂，分屬不同 x 欄）；球場內部虛線／熱區點間的 2px 斷帶屬同一圖形
  內部，皆非驗收對象。
- 5 行球種時球場自動縮放有正確觸發（0.83→約 0.79，未撞 0.78 下限），
  縮圖人工目檢無任何重疊。
